...
def main():
    ...
    # y = (torch.tensor(imread('layer5.png')).permute(2, 0, 1)[None, ...] / 255.).cuda()
    # y = (torch.tensor(imread('out.png')).permute(2, 0, 1)[None, ...] / 255.).cuda()
    y = (torch.tensor(imread('out2.png')).permute(2, 0, 1)[None, ...] / 255.).cuda()
    

    # To compute BRISQUE score as a measure, use lower case function from the library
    brisque_index: torch.Tensor = piq.brisque(y, data_range=1., reduction='none')
